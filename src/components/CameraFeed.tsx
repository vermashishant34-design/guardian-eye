...
/**
 * Live camera feed: uses the real webcam for detection, but renders
 * a privacy-preserving stylized visualization (no actual video shown).
 */
export default function CameraFeed({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (isRunning) {
        // Scale face coords from native video size into 640x480 canvas
        const video = videoRef.current;
        const sx = video?.videoWidth ? 640 / video.videoWidth : 1;
        const sy = video?.videoHeight ? 480 / video.videoHeight : 1;

        faces.forEach((face, i) => {
          const x1 = face.topLeft[0] * sx;
          const y1 = face.topLeft[1] * sy;
          const x2 = face.bottomRight[0] * sx;
          const y2 = face.bottomRight[1] * sy;
          const w = x2 - x1;
          const h = y2 - y1;
          const cx = x1 + w / 2;
          const cy = y1 + h / 2;
          ...
          ctx.fillStyle = i === 0 ? "rgba(34, 197, 94, 0.18)" : "rgba(239, 68, 68, 0.18)";
          ctx.fill();
        ...
        ctx.fillText(`SHIELDEYE v2.0  |  ${now.toLocaleTimeString()}  |  LIVE`, 12, 20);
    ...
    if (isRunning) {
      ...
      ctx.fillText(
        isLoading ? "Loading AI model..." : "Camera inactive",
        320,
        240,
      );
      ctx.textAlign = "start";
  ...
  }, [faces, isThreat, isRunning, isLoading, videoRef]);
  ...
  return (
    <div className="relative rounded-xl overflow-hidden border border-border bg-card">
      ...
      {/* Hidden real video — used by the detection model only, never displayed */}
      <video
        ref={videoRef}
        muted
        playsInline
      ...
      {isRunning && (
        <div className="absolute top-3 right-3 z-30 px-2 py-1 rounded bg-destructive/20 border border-destructive/40">
          <span className="text-[10px] font-mono text-destructive font-bold tracking-wider">LIVE</span>
        </div>
      )}
    </div>
  );
